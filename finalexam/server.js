...
app.set('layout', false); // Disable default layout globally to prevent breaking existing self-contained views

// ─── Static Files ────────────────────────────────────────────
app.use(express.static(path.join(__dirname, 'public')));

// ─── Body Parser read html from data ─────────────────────────────────────────────
...
// On-Sale Products Page Route (Express & Mongoose query)
app.get('/onsale-products', async (req, res) => {
    try {
        // Query MongoDB to find all products where sale is active (isOnSale: true)
        const products = await Product.find({ isOnSale: true }).sort({ createdAt: -1 });

        // Render onsale.ejs inside our layout structure using express-ejs-layouts
        res.render('onsale', {
            products,
            layout: 'layout' // Renders views/onsale.ejs inside views/layout.ejs
        });
    } catch (err) {
        console.error('Onsale products page error:', err);
        res.status(500).render('error', { message: 'Error loading on-sale products', error: err });
    }
});
